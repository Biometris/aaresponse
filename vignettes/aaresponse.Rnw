...
In the global search for a more sustainable way of feeding the human
...
allParams.uncur <- extractParameters(allWoodCurves)
allParams <- curateParameters(allParams.uncur)
...
\begin{figure}[h]
\centering
<<raw1,echo=TRUE,cache=TRUE,fig.height=4,fig.width=10,out.width='\\textwidth'>>=
...
\begin{figure}[tb]
\centering
<<raw2,echo=TRUE,cache=TRUE,fig.height=6,fig.width=10,out.width='.8\\textwidth'>>=
...
\begin{figure}[tb]
\centering
<<raw3,echo=TRUE,cache=TRUE,fig.height=6,fig.width=10,out.width='.8\\textwidth'>>=
...
\begin{figure}[tb]
\centering
<<pcaSc1,echo=FALSE,cache=TRUE,fig.height=5,fig.width=10,out.width='\\textwidth'>>=
PCApar1 <- doPCA(supro, type = "par")
...
\begin{figure}[tb]
\centering
<<pcaL1,echo=FALSE,cache=TRUE,fig.height=8,fig.width=14,out.width='\\textwidth'>>=
...
\begin{figure}[tb]
\centering
<<pcaSc2,echo=FALSE,cache=TRUE,fig.height=5,fig.width=10,out.width='\\textwidth'>>=
PCApro <- doPCA(supro, type = "pro")
...
\begin{figure}[tb]
\centering
<<pcaL2,echo=FALSE,cache=TRUE,fig.height=5,fig.width=8,out.width='.8\\textwidth'>>=
plotLoadings(PCApro)
@
...
\begin{figure}[tb]
\centering
<<pcaSc3,echo=FALSE,cache=TRUE,fig.height=5,fig.width=10,out.width='\\textwidth'>>=
...
\begin{figure}[tb]
\centering
<<pcaL3,echo=FALSE,cache=TRUE,fig.height=5,fig.width=8,out.width='.8\\textwidth'>>=
plotLoadings(PCAaa)
@
...
\begin{figure}[tb]
\centering
<<pcaSc4,echo=FALSE,cache=TRUE,fig.height=5,fig.width=10,out.width='\\textwidth'>>=
...
\begin{figure}[tb]
\centering
<<pcaL4,echo=FALSE,cache=TRUE,fig.height=5,fig.width=8,out.width='.8\\textwidth'>>=
...
be done as follows (for the sake of the example we'll only fit the
amino-acid totals curves for Participants 1 and 2 and a subset of
amino acids):

<<fitWood,eval=FALSE>>=
idx <- supro$Participant == 1 | supro$Participant == 2
resultP12 <- fitWoodAll(supro[idx, 1:8])
@ 

<<fitWoodP12, echo=FALSE,cache=TRUE>>=
<<fitWood>>
@

\noindent
The result is a simple data frame (output limited here to two decimal places):

<<fitWoodShow>>=
format(resultP12[1:6,], digits = 2, nsmall = 2)
@

\noindent
Note that argument \code{what} is available limiting the curve
fitting to either single amino acids, or amino-acid totals (not present
in the example data set). The default is to fit curves through all
suitable columns detected.
...
<<curateFits>>=
curateFits()
resultP12.cur <- curateFits(resultP12)
@ 

\noindent
If other values than the defaults need to be used it can simply be
done by providing boundaries for the parameters \code{a}, \code{m},
\code{c} and/or \code{d}:

<<curateFitsSpecific,eval=FALSE>>=
resultP12.cur2 <- curateFits(resultP12, a = c(0, 500), m = c(0, 50))
@ 

\noindent
Function \code{curateFits} returns a message mentioning how many
records have been curated; in the above two examples no outlying cases
were found.
...
baseline. An example depicting a larger number of amino acids is shown
in Figure~\ref{fig:showDataFits2}.

\begin{figure}[tb]
\centering
<<showDataFits,fig.height=6,fig.width=10,out.width='.8\\textwidth'>>=
showDataFits(supro[idx, 1:8], resultP12.cur,
             subset = AA %in% c("His", "Asn"))
@
\caption{Wood curves for two subjects and two AAs, including the
  measured points.}
...
@
\caption{Wood curves for all available AA traces of two subjects,
  without experimental data, and with extimated baselines subtracted.}
\label{fig:showDataFits2}
\end{figure}

<<fitAllParamsReally,echo=FALSE,message=FALSE,cache=TRUE>>=
resultPall.uncurated <- fitWoodAll(supro, what = "aas")
resultPall <- curateFits(resultPall.uncurated)
allParams.uncurated <- extractParameters(resultPall)
...
estimated values. Figure~\ref{fig:cFits} shows an example, this time
using \code{resultPall}, an \R\ object containing the fits for all
individual amino acids and all participants.

\begin{figure}[tb]
\centering
<<cFits,fig.height=4.5,fig.width=10,out.width='\\textwidth'>>=
showParameters(resultPall, subset = AA %in% c("His", "Asn"))
@
\caption{Fitted curve parameters for all \code{His} and \code{Asn}
  data. Clearly one can see that peaks from Corn come later than from
  \code{BP}, whereas \code{WP} gives the fastest response: curve parameter $m$
  corresponds to the time to the peak maximum.}
\label{fig:cFits}
\end{figure}
...
<<extractParameters>>=
getAUC(resultP12.cur[1,], maxT = 180)
getHeight(resultP12.cur[1,])
getTime2Max(resultP12.cur[1,], minutes = TRUE)
@ 
...
<<extractParametersWrapper>>=
paramsP12 <- extractParameters(resultP12.cur, maxT = 180)
format(paramsP12[1:15,], digits = 2, nsmall = 2)
@
...
<<curatePeaks>>=
curateParameters()
paramsP12.curated <- curateParameters(paramsP12)
@ 
...
showPoIs(AUC = summarizePoIs(allParams, "AUC"),
         Height = summarizePoIs(allParams, "Height"),
         Time2Max = summarizePoIs(allParams, "Time2Max"),
         subset = AA %in% c("Asn", "His"))
@


\begin{figure}[tb]
\centering
<<pois,echo=FALSE,cache=TRUE,fig.height=2.5,fig.width=6,out.width='.6\\textwidth'>>=
<<showPoIs>>
@
...
<<compareInterventions,cache=TRUE,message=FALSE,warning = FALSE>>=
finalResult <- compareInterventions(allParams, target = "AUC",
                                    logTransform = TRUE)
@ 
...
or 0 in case of differences).

\begin{figure}[tb]
\centering
<<CIs,echo=TRUE,cache=TRUE,message=FALSE,fig.height=5,fig.width=8,out.width='.8\\textwidth'>>=
showCIs(finalResult)
@
...
\begin{figure}[tb]
\centering
<<CIs1,echo=TRUE,cache=TRUE,message=FALSE,fig.height=5,fig.width=8,out.width='.8\\textwidth'>>=
...
\begin{figure}[tb]
\centering
<<CIs2,echo=TRUE,cache=TRUE,message=FALSE,fig.height=5,fig.width=8,out.width='.8\\textwidth'>>=
...
\begin{figure}[tb]
\centering
<<CIs3,echo=TRUE,cache=TRUE,message=FALSE,fig.height=5,fig.width=8,out.width='.8\\textwidth'>>=
...
\begin{figure}[tb]
\centering
<<combinedCIs,fig.height=8,fig.width=10,out.width='\\textwidth'>>=
...
<<imputeVals,echo=TRUE,cache=TRUE>>=
...
\begin{figure}[tb]
\centering
<<imputedValues,echo=FALSE,cache=TRUE,fig.height=10,fig.width=10,out.width='\\textwidth'>>=
...
\caption{Top row: fitted against imputed values for \code{AUC} and
  \code{Height} -- these imputed values are not used since the values
  based on the curve fits are available.
  Bottom row: distribution of imputed values corresponding to missing PoIs.}
\label{fig:imputedValues}
\end{figure}

<<getImputedCIs, echo=FALSE,cache=TRUE, message=FALSE, warning=FALSE>>=
...
\begin{figure}[tb]
\centering
<<allCIs,fig.height=8,fig.width=7,out.width='.7\\textwidth'>>=
