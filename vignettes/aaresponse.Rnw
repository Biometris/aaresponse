...
\begin{figure}[tb]
\centering
<<pcaL1,echo=FALSE,fig.height=9,fig.width=16.5,out.width='\\textwidth'>>=
...
Finally, it should be mentioned that the traditional way of obtaining
PoIs -- i.e., without fitting Wood curves -- is available through
function \code{PoItradAll}. It operates directly on the data frame
containing the original data, and returns an object of class
\code{'aar'}, just like the one obtained from
\code{extractParameters}. Check the manual page for more information.
...
the correlation is less good, but still usable.

To show how it works, we again look at the supro data, but instead of
focusing on the totals (which contain very few missing values) we
analyse the essential amino acids, and apply quite aggressice curation
(in particular for the \code{c} parameter). For computational reasons we only
include the two participants that show the highest number of missing values:

<<imputeVals,echo=TRUE>>=
idx <- which(supro$Participant %in% c(4, 9))
resultPaas.uncurated <- fitWoodAll(supro, what = 'essentials')
resultPaas <- curateFits(resultPaas.uncurated)
aaParams.uncurated <- extractParameters(resultPaas)
aaParams <- curateParameters(aaParams.uncurated)

fullPoIs <- imputePoIs(supro, aaParams, quant = .2)
tail(fullPoIs)
@ 
...
small in almost all cases, as they should be. Using these imputed
values in the comparison of the interventions then leads to the CIs shown in 
Figure~\ref{fig:allCIs}.
