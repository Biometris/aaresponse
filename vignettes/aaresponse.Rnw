...
curves through all suitable columns detected (\code{``all''}).
...
@
\caption{Wood curves for two subjects, including the measured points.}
...
@
\caption{Fitted curve parameters. Clearly one can see that peaks from Corn
...
other values are more appropriate. These can simply be provided to the
...
\noindent
With these settings, no records are curated, which is in agreement
with Figure~\ref{fig:showDataFits}, where no really big outliers are
visible. At this point it is important not to be too strict -- at the
...
\begin{figure}[tb]
\centering
<<pois,echo=FALSE,fig.height=2.5,fig.width=6,out.width='.6\\textwidth'>>=
<<showPoIs>>
@
...
\begin{figure}[tb]
\centering
<<CIs,echo=TRUE,message=FALSE,fig.height=5,fig.width=8,out.width='.8\\textwidth'>>=
showCIs(finalResult)
@
...
\begin{figure}[tb]
\centering
<<CIs1,echo=TRUE,message=FALSE,fig.height=5,fig.width=8,out.width='.8\\textwidth'>>=
...
\begin{figure}[tb]
\centering
<<CIs2,echo=TRUE,message=FALSE,fig.height=5,fig.width=8,out.width='.8\\textwidth'>>=
...
\begin{figure}[tb]
\centering
<<CIs3,echo=TRUE,message=FALSE,fig.height=5,fig.width=8,out.width='.8\\textwidth'>>=
